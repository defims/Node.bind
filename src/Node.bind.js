...
NodeBind.setTreeScope   = function(node, scope){
    ...
    NodeBind.walkTree( node, function(depth){
        currNode    = this;
        if((nbViews = currNode.nbViews) && (nbModels = currNode.nbModels)){
            for(k in nbViews){
                nbView    = nbViews[k];
                directive = nbView.directive.origin;
                //ignore root node's scope and repeat because their scope comes from parent
                if( !(currNode == node && directive == 'scope' && directive == 'repeat') ){
                    bindings        = nbView.bindings;
                    //unbind nbModel and nbView
                    while(bindings.length){
                        nbModel = bindings.pop().target;
                        nbModel.unbind(nbView);
                    }
                    //clean all models
                    nbModels    = currNode.nbModels   = [];
                    //clean view bindings
